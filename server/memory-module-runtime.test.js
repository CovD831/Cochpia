...
// C-11 (R-020 2a): a Memory context now requires a calling agent. These
// fixtures exercise the compatibility surface, which represents an
// application-level caller rather than a chat session, so the resolver stands
// in for the session lookup the app wires in index.js.
const request = { body: {}, query: {}, get: () => undefined };
const testResolveAgentId = () => 'test-agent';
const buildRuntime = options => createMemoryModuleRuntime({ resolveAgentId: testResolveAgentId, ...options });
...
test('Memory Module runtime migrates legacy memories once and removes the legacy field', async () => {
  ...
  let persists = 0;
  const runtime = buildRuntime({ getState: () => state, persistState: async () => { persists += 1; } });
...
test('Memory Module compatibility mutations operate on canonical assertions', async () => {
  ...
  const runtime = buildRuntime({ getState: () => state });
  const compatibility = runtime.compatibilityForRequest(request);
