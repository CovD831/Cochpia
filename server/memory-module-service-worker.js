...
// Standalone Memory Module service worker.
//
// This module was deleted in d3a6d81 ("R-020 stage 4 - delete unused endpoints
// and dead modules") on the (incorrect) audit finding that it had "zero
// non-test references". The audit only covered the `server/` tree and missed
// the static import from `services/memory-module/index.js` (lines 13 and 232),
// so the deletion broke the standalone service at module-load time: it could
// not even start from HEAD.
//
// The upstream intent was to retire the *extraction* role into the in-process
// `createMemoryExtractionDrain` (memory-extraction.js). But that drain is
// single-tenant (bound to one (tenant, subject) context), serializes per
// subject, and only writes `index_documents` rows when an embedding gateway is
// present. The standalone service is multi-tenant and driven by the PostgreSQL
// outbox with cross-process leases, so it needs this multi-tenant outbox
// worker -- not the in-process drain. Every dependency below is a live,
// tested module, so restoring this file reintroduces no dead code.
//
// Responsibilities (restored, equivalent to the pre-deletion behaviour):
//   - poll the `memory_outbox_events` table for `memory-derived` events
//     (raw_event.created / assertion.active) across all tenants
//   - extract durable facts from raw events (processExtractionEvent) when
//     autoExtract is enabled
//   - rebuild the lexical/vector index (rebuildIndexDocumentsAsync, embedding
//     optional -> BM25 fallback) when hybridRetrieval/vectorRetrieval is on
//   - project the stable profile and rebuild episodes behind their flags
//   - run lifecycle retention sweeps on an interval
//   - fence derived writes with the claimed outbox lease (createMemoryModuleWorker)
