...
1. API service: use `deploy/railway-api.toml`, set `PORT`, one or more HTTPS `CLIENT_ORIGIN` values, `STORAGE_PROVIDER=postgres`, Supabase database variables, `AUTH_MODE=required`, model provider variables, and Memory Module variables. Production rejects HTTP or missing CORS origins.
