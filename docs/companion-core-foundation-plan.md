...
本节是对本计划的实施约束补充，不改变“Memory-first Companion Core”的总体方向。它把目标架构收敛为第一条可交付、可验证的聊天底座。

### 13.1 定位

Cochpia 的底座不是“一个带记忆功能的聊天页面”，而是：
...
- Memory Foundation 负责交互事件、长期记忆、Scope、敏感度、版本、检索、治理和恢复。
- Companion Core 负责身份、会话、当前状态、Context Builder、Model Gateway、聊天轮次和结果提交。
- Extension Modules 负责游戏、任务、日历、音乐及未来的外部 Agent；它们拥有自己的领域状态，通过事件和受限上下文接入 Core。

Memory 是连续性的基础，但不拥有聊天回复、游戏世界或全部应用状态。
...
- moduleId 和契约版本；
- 自有 command 及授权边界；
- 自有 event type、schema version 和 producer；
- 自有 canonical state 及唯一写入口；
- 可选的 contextContributor，只返回 bounded、policy-filtered view；
- projection/replay handler；
- export、forget、delete 和恢复钩子；
- 与 Core v0 的兼容和移除条件。

模块不得：

- 直接读取或修改 MemoryModule.state；
- 直接写长期记忆表；
- 伪造 tenant/user/agent/relationship 字段；
