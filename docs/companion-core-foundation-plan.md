...
> 实施审计快照（2026-08-25）：Phase 0–2、Phase 4–5 的主要代码闭环已落地；主应用级 export operation、固定快照、删除传播清单、LifeState policy-filtered context，以及 LifeState 治理操作账本/重试 reconciliation 也已接入。聊天、LifeState、任务、日历、音乐和兼容 Memory API 的外部写入现在均有统一 Collector 边界；LifeState、任务、日历和音乐通过应用侧 Interaction Outbox 保证跨存储失败时可重试，LifeState pending command 在缺失 outbox 时可按 session provenance 重建，任务/日历在入队或持久化失败时回滚领域变更，music outbox 持久化失败返回稳定可重试 503 并保留 pending evidence，session delete 先持久化主应用删除、Memory 失败时恢复主应用状态，account delete 在 Memory 删除已提交而主应用最终保存失败时恢复主应用快照并保留 Memory 删除账本，随后可用同幂等键重试完成清理，MemoryModuleRuntime 在 canonical memory state 被替换后自动 rebind，chat stream 与 group chat 的 Collector/Context 预检失败均 fail-closed 并回滚未完成的本地写入；聊天 provider、工作模式工具调用和 Pi RPC 现在统一经过 Companion Model Gateway 的 secret 输入/输出门禁；Pi RPC 以 text-only 模式运行，工具请求被拒绝并回退到审批控制的本地工具链；导入和同步先完成 legacy memory migration；账户/会话删除在活动交互期间 fail-closed；MCP/API memory write 通过 canonical ingress 传递幂等 provenance，上传文件按用户 storage key 隔离并纳入导出/删除治理。Memory outbox 新增 `next_attempt_at` 指数退避，避免 outage 时 backlog 饥饿。聊天 run 状态机、SSE cursor 边界和独立 Memory Module signed service-auth v1（含 PostgreSQL nonce replay ledger）已接入。兼容 `/api` 现在覆盖候选列表/激活、确认/拒绝、修正、Pin/Unpin、撤回、忘记、删除和导出；消息编辑通过 source revision 进入 canonical event，reconciliation 只认最新 revision，删除会清理同消息全部 revision。当前仓库级回归为 362 tests（357 passed、5 skipped、0 failed），其中包含 LifeState 离线命令队列、Companion Model Gateway、Pi RPC Gateway、Pi client policy、MCP/API ingress、上传治理、Memory replay provenance、记忆生命周期和聊天编辑 acceptance；`npm run build` 通过；本轮真实 PostgreSQL、required Auth、TLS、双进程 worker、Memory acceptance、独立 SDK、custom-format backup/restore、隔离 PITR/tombstone replay、虚拟时钟 outage/backlog、lean 1M pgvector/HNSW 和 fixture Model Gateway audit 证据见 [`artifacts/companion-core-acceptance-rerun-2026-08-24.json`](../artifacts/companion-core-acceptance-rerun-2026-08-24.json)、[`artifacts/companion-core-outage-backlog-2026-08-24.json`](../artifacts/companion-core-outage-backlog-2026-08-24.json)、[`artifacts/memory-postgres-vector-benchmark-1m-lean-2026-08-24.json`](../artifacts/memory-postgres-vector-benchmark-1m-lean-2026-08-24.json) 和 [`artifacts/memory-model-gateway-audit-2026-08-24.json`](../artifacts/memory-model-gateway-audit-2026-08-24.json)。以下 Alpha/生产门禁仍保持未通过，不能因为本机 smoke 通过而宣称生产就绪：真实/脱敏 600-case 评测、完整 canonical 1M pgvector/HNSW、生产 RPO/RTO/长期故障注入、正式 Model Gateway 供应商审计，以及正式托管 HTTPS/日志/缓存验证。
