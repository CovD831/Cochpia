...
2. **主体 Agent 由用户创建**：用户通过「新建角色」创建的 agent（弦、Cody…）才是可对话的主体。
...
- 老用户已有 agent（弦、Cody）默认 `primary` 缺失 → 视为 `false`；用户可到资料页勾选「主陪伴」。
...
*执行前先读 `docs/runtime-refactor-audit.md` 了解当前目录结构；如行号有出入以实际代码为准。完成后停在待弦复查，不要自行扩展范围。*
