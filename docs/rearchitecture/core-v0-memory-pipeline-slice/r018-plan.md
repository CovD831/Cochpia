...
### 7.7 状态：**止损（2026-09-09，R018-AR-008）**

- [x] 对抗性审查（7 findings 全部 consumed）
- [x] 设计修订 v2
- [x] P-1 探针两轮（第一轮度量含实现错误：换值对被误计为同指称，已按
  本节定义修正后重跑）
- [x] **门线判定：止损**——跨轮 norm 一致率 18/21（85.7%）< 90%。

失败明细（3 条，全部是"看似可桥、实则不同实体"）：
健身卡/健身房、直属领导/部门领导、日语/五十音图。前缀词干（健身/领导）
能凑齐字面，但会制造**错误链接**（卡≠房、直属≠部门）——正是 R-015
教训的镜像：字面桥接的召回是用精度买的。

**P-1 的正面产出（保留价值）**：实体抽取本身质量可用（一跳桥接配
tagger 提示词 3/4；抽取命中率 7/9，其中 2 MISS 疑为探针标注误差）。
卡死的只有跨轮归一——需要 embedding 级实体链接（成本与 false-bridge
风险另评）。

**处置**：R-018 搁置（设计、探针、教训全部保留在案）。future work
立项条件：出现真实的实体点名词检索需求，且愿意为语义实体链接引入
embedding 成本时，以本探针为验收工具重启。