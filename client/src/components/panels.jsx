...
export function AgentFormModal({ agent, models, onClose, onSave }) {
  const [draft, setDraft] = useState({ name: agent?.name || '', avatarImage: agent?.avatarImage || '', remark: agent?.remark || '', role: agent?.role || '朋友', tone: agent?.tone || '自然、温和', signature: agent?.signature || '', persona: agent?.persona || '', provider: agent?.provider || '', model: agent?.model || '', memoryNotes: agent?.memoryNotes || '', primary: agent?.primary || false });
