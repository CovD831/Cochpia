...
import { SettingsBody } from './workspace/SettingsWindow';
...
// AR-212 韧性：refresh() 的 8 个接口改为逐接口隔离（见 refresh）。任一接口失败
// 不再整批 reject 拖垮整页；失败面板用 PanelError 就地展示，避免静默空白。
function panelErrorMessage(name, reason) {
  const detail = reason && reason.message ? reason.message : '网络或服务器异常';
  return `${name}加载失败：${detail}`;
}
...
function App() {
  ...
  // AR-212 修复：8 个面板接口逐接口隔离。任一接口失败只影响自己的面板，
  // 不拖垮整批（旧实现用 Promise.all，单点 400 会整页 reject 导致静默空白）。
  // 调用方式 / 顺序 / 参数保持不变，仅把聚合从 all 改为 allSettled。
  const refresh = async () => {
    const [sessionsR, memoryR, personalityR, evidenceR, historyR, modelsR, presetsR, agentsR] = await Promise.allSettled([
    ...
    const nextSessions = valueOf(sessionsR);
    const nextMemory = valueOf(memoryR);
    const nextPersonality = valueOf(personalityR);
    const nextEvidence = valueOf(evidenceR);
    const nextHistory = valueOf(historyR);
    const modelCatalog = valueOf(modelsR);
    const nextPresets = valueOf(presetsR);
    const nextAgents = valueOf(agentsR);
    ...
    // 记录每个失败面板的可见错误；失败面板渲染为空 + 错误提示，而非静默空白。
    const nextPanelErrors = {};
    if (sessionsR.status !== 'fulfilled') nextPanelErrors.sessions = panelErrorMessage('会话列表', sessionsR.reason);
    if (memoryR.status !== 'fulfilled') nextPanelErrors.memory = panelErrorMessage('共享记忆', memoryR.reason);
    if (personalityR.status !== 'fulfilled') nextPanelErrors.personality = panelErrorMessage('人格', personalityR.reason);
    if (evidenceR.status !== 'fulfilled') nextPanelErrors.growthEvidence = panelErrorMessage('成长证据', evidenceR.reason);
    if (historyR.status !== 'fulfilled') nextPanelErrors.personalityHistory = panelErrorMessage('人格版本', historyR.reason);
    if (modelsR.status !== 'fulfilled') nextPanelErrors.models = panelErrorMessage('模型目录', modelsR.reason);
    if (presetsR.status !== 'fulfilled') nextPanelErrors.atmospherePresets = panelErrorMessage('氛围预设', presetsR.reason);
    if (agentsR.status !== 'fulfilled') nextPanelErrors.agents = panelErrorMessage('好友 Agent', agentsR.reason);
  ...
  return <><BackgroundLayer /><GroupInfoPanel session={currentSession} agents={agents} open={groupPanelOpen} onClose={() => setGroupPanelOpen(current => !current)} onSave={updateCurrentGroup} onInvite={inviteAgent} onRemove={removeGroupAgent} onAgentSave={saveAgent} onAgentRemove={removeAgent} /><div className={`app-shell${minimized ? ' minimized' : ''}`}>
    ...
    <div className="model-dock">
      ...
      <button className="icon-button compact" aria-label="Open music" title="Open music" onClick={openMusic}>♫</button><button className="icon-button compact" aria-label="打开设置" title="打开设置" onClick={() => setPage('settings')}>⚙</button>
    </div>
